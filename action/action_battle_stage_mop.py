...
def log(user, stage_index, need_stamina, cost_item_str, add_exp, add_gold, monster_str, equip_str, item_str):
    """
        输出日志
    """
...
def parse(log_part_lst):
    ...
    result['add_exp'] = int(log_part_lst[4])
    result['cur_exp'] = int(log_part_lst[5])
    result['add_gold'] = int(log_part_lst[6])
    result['cur_gold'] = int(log_part_lst[7])
    result['add_monster_list'] = action_base.get_val(log_part_lst, 8, [], True)
    result['add_equip_list'] = action_base.get_val(log_part_lst, 9, [], True)
    result['add_item_list'] = action_base.get_val(log_part_lst, 10, [], True)
