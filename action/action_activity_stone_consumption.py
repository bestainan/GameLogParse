...
def log(user, activity_tid, add_gold, add_stone, add_free_draw, equip_str, item_str):
    """
        输出日志
    """
    action = game_define.EVENT_ACTION_ACTIVITY_STONE_CONSUMPTION_REWARD
...
def parse(log_part_lst):
    ...
    result['add_equip_list'] = action_base.list_parse(log_part_lst[8])
    result['add_item_list'] = action_base.list_parse(log_part_lst[9])
