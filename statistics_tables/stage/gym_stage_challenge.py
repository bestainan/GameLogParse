...
def get_table(search_start_date, search_end_date, register_start_date=None, register_end_date=None, channel_id=-1, server_id=-1):
    ...
    # 截取日志到 副本事件
    stage_action_lst = [
        game_define.EVENT_ACTION_GYM_WIN,
        game_define.EVENT_ACTION_GYM_FAIL
    ]
    ...
    for _stage_index, _logs in stage_action_log_dict.items():
        #拆分日志 - 胜利部分
        _win_logs = daily_log_dat.filter_logs(_logs, action=game_define.EVENT_ACTION_GYM_WIN)
        #拆分日志 - 失败部分
        _fail_logs = daily_log_dat.filter_logs(_logs, action=game_define.EVENT_ACTION_GYM_FAIL)
        #拆分日志 - 扫荡部分
        _mop_logs = daily_log_dat.filter_logs(_logs, action=game_define.EVENT_ACTION_GYM_MOP)
        # 副本难度
