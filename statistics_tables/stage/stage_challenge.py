...
def get_table(search_start_date, search_end_date, register_start_date=None, register_end_date=None, channel_id=-1, server_id=-1):
    ...
    # # 截取日志到 副本事件
    # stage_action_lst = [
    #     game_define.EVENT_ACTION_STAGE_NORMAL_FAIL,
    #     game_define.EVENT_ACTION_STAGE_NORMAL_WIN,
    #     game_define.EVENT_ACTION_STAGE_MOP,
    # ]
    #
    ...
    #
    # # 遍历所有的关卡日志
    # type_name_lst = [u'普通副本',u'精英副本',u'英雄副本',u'经验副本',u'金币副本',u'抓宠副本',u'世界BOSS副本',u'修行']
    # table_lst = []
    # for _stage_index, _logs in stage_action_log_dict.items():
    #     #拆分日志 - 胜利部分
    #     _win_logs = daily_log_dat.filter_logs(_logs, action=game_define.EVENT_ACTION_STAGE_NORMAL_WIN)
    #     #拆分日志 - 失败部分
    #     _fail_logs = daily_log_dat.filter_logs(_logs, action=game_define.EVENT_ACTION_STAGE_NORMAL_FAIL)
    #     #拆分日志 - 扫荡部分
    #     _mop_logs = daily_log_dat.filter_logs(_logs, action=game_define.EVENT_ACTION_STAGE_MOP)
    #     # 副本难度
    #     stage_config = game_config.get_stages_config(_stage_index)
    #     # 副本名称
    #     stage_name = stage_config['stageInfo']
    #     # 副本类型
    #     stage_type_name = type_name_lst[stage_config['type']-1]
    #     # 挑战次数
    ...
    #     row_lst = [stage_type_name, stage_name, challenge_num, win_num, mop_num, win_rate, first_challenge_num,
    #                first_challenge_win]
