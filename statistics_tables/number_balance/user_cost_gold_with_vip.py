...
def get_table(search_start_date, search_end_date, channel_id=-1, server_id=-1, player_min_level=1,
    ...
    # 根据事件拆分消耗日志
    action_cost_gold_dict = daily_log_dat.split_log_action_logs(all_cost_gold_log_lst)

    table_lst = []
    # 遍历所有事件
    for _action, _logs in action_cost_gold_dict.items():
        # 事件名称
        ...
        # vip0 消耗钻石
        vip0_cost_gold_num = daily_log_dat.get_sum_int_with_key(vip0_user_log, 'cost_gold')
        # 消耗钻石数量
        total_cost_gold_num = daily_log_dat.get_sum_int_with_key(_logs, 'cost_gold')
        ...
        for i in xrange(1, 13):
            ...
            vip_x_cost_gold_num = daily_log_dat.get_sum_int_with_key(vip_x_log, 'cost_gold')
            vip_x_cost_gold_user_num = daily_log_dat.get_set_num_with_key(vip_x_log, 'uid')
