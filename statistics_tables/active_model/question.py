...
def get_table(search_start_date, search_end_date, register_start_date=None, register_end_date=None, channel_id=-1, server_id=-1):
    ...
    # 截取日志到 副本事件
    stage_action_lst = [
        game_define.EVENT_ACTION_QUIZ_ANSWER_TRUE
    ]
    ...
    for _day in xrange(search_days):
        ...
        # 过滤出日志
        active_log_lst = daily_log_dat.filter_logs(row_logs,
                                                     function=lambda log: log['action'] in stage_action_lst)
        # 日期
        row_date_str = row_date.strftime('%m/%d/%Y')
        # 参与人数
        join_user_num = daily_log_dat.get_set_num_with_key(active_log_lst, 'uid')
        # 总次数
        total_active_num = len(active_log_lst)
        # 行日期之前的 日志
        until_row_day_logs = daily_log_dat.filter_logs(new_log_lst, function=lambda log: log['log_time'].date() <= row_date)
        # 到达要求人数
        can_join_user_num = daily_log_dat.get_set_num_with_key(until_row_day_logs, 'uid', function=lambda log:log['level'] >= 26)
        # 参与率
        join_rate = 0
