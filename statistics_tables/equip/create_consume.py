...
def get_create_table(search_start_date, search_end_date, equip_tid_lst, register_start_date=None, register_end_date=None, channel_id=-1,
    ...
    create_equip_log_lst = daily_log_dat.filter_logs(new_log_lst, function=lambda log: is_equip_tid_in_key(log, 'add_equip_list', equip_tid_lst))
    print("create_equip_log_lst: "+str(create_equip_log_lst))
    # 用装备TID 拆分日志
    equip_tid_dict = dict()
    action_set = set()
    for _log in create_equip_log_lst:
        add_equip_list = _log['add_equip_list']
        action_set.add(_log['action'])
        for i in range(0, len(add_equip_list), 2):
            _tid = add_equip_list[i]
            # _num = add_equip_list[i + 1]
            _key = _tid
            if _key in equip_tid_dict:
                equip_tid_dict[_key].append(_log)
            else:
                equip_tid_dict[_key] = [_log]

    # 遍历所有的怪
    table_lst = []
    head_name_lst = []
    for _tid in equip_tid_lst:
        row_lst = []
        _name = _tid
        _key = _tid
        _log_lst = equip_tid_dict.get(_key, [])
        if _log_lst:
            # 遍历当前的怪星级的日志
...
def get_consume_table(search_start_date, search_end_date, equip_tid_lst, register_start_date=None, register_end_date=None, channel_id=-1,
              server_id=-1, player_min_level=1,
              player_max_level=999):
    #获取搜索区间日志
    new_log_lst = daily_log_dat.get_new_log_lst(search_start_date, search_end_date)
    ...
    remove_equip_log_lst = daily_log_dat.filter_logs(new_log_lst, function=lambda log: is_equip_tid_in_key(log, 'remove_equip_list', equip_tid_lst))

    # 用装备TID 拆分日志
    equip_tid_dict = dict()
    action_set = set()
    for _log in remove_equip_log_lst:
        remove_equip_list = _log['remove_equip_list']
        action_set.add(_log['action'])
        for i in range(0, len(remove_equip_list), 2):
            _tid = remove_equip_list[i]
            # _num = add_equip_list[i + 1]
            _key = _tid
            if _key in equip_tid_dict:
                equip_tid_dict[_key].append(_log)
...
def is_equip_tid_in_key(log, key, equip_tid_lst):
    """
        怪在日志中
    """
    if key in log:
