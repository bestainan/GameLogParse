...
def start(split_date):
    ...
    for _server_id in LOCAL_LOG_PATH_NAME_LST:
        try:
            ...
            # 玩家首冲情况
            _output_user_recharge_state(out_put_file_path, split_date, _server_id)
...
def _output_user_recharge_state(out_put_file_path, split_date, _server_id):
    print("USER_RECHARGE_STATE")
    result = get_table(split_date, channel_id=-1, server_id=_server_id)

    out_put_file = open(out_put_file_path + 'USER_RECHARGE_STATE', 'w')
    pickle.dump(result, out_put_file)
    out_put_file.close()


def get_recharge_shop_index_uid_num(spe_column, row_date, specfy_column_name='shop_index', shop_index=-1, channel_id=-1, server_id=-1):
    """
        指定日期指定充值金额档充值人数
    """
    channel_server_str = ""
    ...
    # 获取次数
    sql = "SELECT count(%s) FROM EVENT_ACTION_RECHARGE_PLAYER where %s  action = %s and log_time >= '%s' and log_time < '%s' %s " % (spe_column, channel_server_str, game_define.EVENT_ACTION_RECHARGE_PLAYER, row_date, row_date + datetime.timedelta(days=1), shop_index_str)
    # print sql
...
def get_table(search_date, channel_id=-1, server_id=-1):
    ...
    for shop_id in xrange(1, 9):
        user_num = get_recharge_shop_index_uid_num('uid', search_date, 'shop_index', shop_id, channel_id, server_id)
        # 插入数据
        row.append(user_num)
