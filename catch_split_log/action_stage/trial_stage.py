...
CUR_ACTION_LST = [game_define.EVENT_ACTION_TRIAL_BATTLE_WIN, game_define.EVENT_ACTION_TRIAL_BATTLE_FAIL]    # 当前行为ID列表
    ...
    for _server_id in LOCAL_LOG_PATH_NAME:
        try:
            ...
            if log_lines:
                ...
                #试炼副本
                _output_TRIAL_STAGE_CHALLENGE(split_date,out_put_file_path)
        except:
            pass



def _output_TRIAL_STAGE_CHALLENGE(split_date,out_put_file_path):
    """
        经验关卡挑战数据
    ...
    # 胜利失败关卡日志字典
    open_level = game_config.get_game_config_val_int("OpenLevelTrial")
